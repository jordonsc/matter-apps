...
void create_hx711_sensor(node_t* node, hx711_sensor* sensor)
{
    ...
    if (configured_hx711_sensors == 0) {
        // Only set this once for the first sensor
    ...
    // Create a pressure sensor endpoint (which provides weight measurements via PressureMeasurement cluster)
    ...
    if (pressure_endpoint == nullptr) {
        ESP_LOGE(TAG, "Failed to create HX711 pressure sensor endpoint");
        return;
    }
    ...
    // Add descriptor features to pressure sensor
    cluster_t* descriptor = cluster::get(pressure_endpoint, Descriptor::Id);
    if (descriptor) {
        descriptor::feature::taglist::add(descriptor);
    }

    // Configure the pressure measurement cluster with proper measurement range
    cluster_t* pressure_cluster = cluster::get(pressure_endpoint, PressureMeasurement::Id);
    if (pressure_cluster) {
        // Set minimum measurable value (full int16_t range for raw HX711 values)
        // After proper calibration with scale factor, values should be in meaningful units
        ...
        // Set maximum measurable value (full int16_t range for raw HX711 values)
        ...
        ESP_LOGI(TAG, "Configured pressure measurement range: -32000 to 32000 (configure scale factor for proper units)");
    }

    // Give the previous endpoint time to fully initialize in NVS
    vTaskDelay(pdMS_TO_TICKS(100));

    // Create an outlet endpoint for tare functionality
    // When turned on, it will tare the scale and then turn itself back off
    ESP_LOGI(TAG, "About to create on/off <outlet> for HX711 tare function");
    ESP_LOGI(TAG, "Node pointer: %p, current sensor endpoint: %d", node, sensor->endpoint);
    ...
    ESP_LOGI(TAG, "on_off_plugin_unit::create returned: %p", tare_endpoint);

    if (tare_endpoint == nullptr) {
        ESP_LOGE(TAG, "Failed to create on/off tare endpoint for HX711 sensor");
        sensor->tare_button_endpoint = 0;
    } else {
        sensor->tare_button_endpoint = endpoint::get_id(tare_endpoint);

        // Add descriptor taglist feature
        ...
        ESP_LOGI(TAG, "Tare outlet created with endpoint_id %d", sensor->tare_button_endpoint);
    }

    ESP_LOGI(TAG, "HX711 sensor created with endpoint_id %d", sensor->endpoint);

    // Try to load tare offset from NVS
    vTaskDelay(pdMS_TO_TICKS(2000));  // Give HX711 time to settle (2 seconds)

    long saved_offset = 0;
    esp_err_t err = load_tare_offset_from_nvs(sensor->endpoint, &saved_offset);

    if (err == ESP_OK) {
        // Loaded saved offset from NVS - use it instead of taring
        ...
        // No saved offset - perform initial tare
